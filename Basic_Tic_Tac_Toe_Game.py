...
reset_button = Button(text = 'New Game', font=('Aerial', 25), command = new_game)
reset_button.pack(side = 'top')
reset_button.config(bg='green')

exit_button = Button(text = 'Exit Game', font=('Aerial', 25), command = exit)
exit_button.pack(side='top')
exit_button.config(bg='grey')
frame = Frame(window)
frame.pack()

for row in range(3):
    for column in range(3):
        buttons[row][column] = Button(frame, text = '', font=('Aerial', 25), 
                                       width = 5, height = 2,
